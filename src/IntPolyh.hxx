...
void preregister_template_IntPolyh_Array(py::object &m, const char *name){
    py::class_<IntPolyh_Array<Type> ,std::unique_ptr<IntPolyh_Array<Type>> >(m,name,R"#(Class IntPolyh_Array (dynamic array of objects))#");
}

template <typename Type>
void register_template_IntPolyh_Array(py::object &m, const char *name){
    static_cast<py::class_<IntPolyh_Array<Type> ,std::unique_ptr<IntPolyh_Array<Type>> >>(m.attr(name))

...
// classes forward declarations only
    ...
    py::class_<TopoDS_Edge ,std::unique_ptr<TopoDS_Edge>  , TopoDS_Shape >(m,"TopoDS_Edge",R"#(Describes an edge which - references an underlying edge with the potential to be given a location and an orientation - has a location for the underlying edge, giving its placement in the local coordinate system - has an orientation for the underlying edge, in terms of its geometry (as opposed to orientation in relation to other shapes).)#");
    ...
    py::class_<TopoDS_Builder ,std::unique_ptr<TopoDS_Builder>  >(m,"TopoDS_Builder",R"#(A Builder is used to create Topological Data Structures.It is the root of the Builder class hierarchy.)#");
    py::class_<TopoDS_Compound ,std::unique_ptr<TopoDS_Compound>  , TopoDS_Shape >(m,"TopoDS_Compound",R"#(Describes a compound which - references an underlying compound with the potential to be given a location and an orientation - has a location for the underlying compound, giving its placement in the local coordinate system - has an orientation for the underlying compound, in terms of its geometry (as opposed to orientation in relation to other shapes). Casts shape S to the more specialized return type, Compound.)#");
    py::class_<TopoDS_Wire ,std::unique_ptr<TopoDS_Wire>  , TopoDS_Shape >(m,"TopoDS_Wire",R"#(Describes a wire which - references an underlying wire with the potential to be given a location and an orientation - has a location for the underlying wire, giving its placement in the local coordinate system - has an orientation for the underlying wire, in terms of its geometry (as opposed to orientation in relation to other shapes).)#");
    ...
    py::class_<TopoDS ,std::unique_ptr<TopoDS>  >(m,"TopoDS",R"#(Provides methods to cast objects of class TopoDS_Shape to be onjects of more specialized sub-classes. Types are verified, thus in the example below, the first two blocks are correct but the third is rejected by the compiler.)#");
    py::class_<TopoDS_Vertex ,std::unique_ptr<TopoDS_Vertex>  , TopoDS_Shape >(m,"TopoDS_Vertex",R"#(Describes a vertex which - references an underlying vertex with the potential to be given a location and an orientation - has a location for the underlying vertex, giving its placement in the local coordinate system - has an orientation for the underlying vertex, in terms of its geometry (as opposed to orientation in relation to other shapes).)#");
    py::class_<TopoDS_Shell ,std::unique_ptr<TopoDS_Shell>  , TopoDS_Shape >(m,"TopoDS_Shell",R"#(Describes a shell which - references an underlying shell with the potential to be given a location and an orientation - has a location for the underlying shell, giving its placement in the local coordinate system - has an orientation for the underlying shell, in terms of its geometry (as opposed to orientation in relation to other shapes).)#");
    py::class_<TopoDS_TShape ,opencascade::handle<TopoDS_TShape> ,Py_TopoDS_TShape , Standard_Transient >(m,"TopoDS_TShape",R"#(A TShape is a topological structure describing a set of points in a 2D or 3D space.A TShape is a topological structure describing a set of points in a 2D or 3D space.A TShape is a topological structure describing a set of points in a 2D or 3D space.)#");

...
// classes forward declarations only
    py::class_<BRepOffsetAPI_NormalProjection ,std::unique_ptr<BRepOffsetAPI_NormalProjection>  , BRepBuilderAPI_MakeShape >(m,"BRepOffsetAPI_NormalProjection",R"#(A framework to define projection onto a shape according to the normal from each point to be projected. The target shape is a face, and the source shape is an edge or a wire.)#");
    py::class_<BRepOffsetAPI_MakeEvolved ,std::unique_ptr<BRepOffsetAPI_MakeEvolved>  , BRepBuilderAPI_MakeShape >(m,"BRepOffsetAPI_MakeEvolved",R"#(Describes functions to build evolved shapes. An evolved shape is built from a planar spine (face or wire) and a profile (wire). The evolved shape is the unlooped sweep (pipe) of the profile along the spine. Self-intersections are removed. A MakeEvolved object provides a framework for: - defining the construction of an evolved shape, - implementing the construction algorithm, and - consulting the result. Computes an Evolved by 1 - sweeping a profil along a spine. 2 - removing the self-intersections.)#");
    py::class_<BRepOffsetAPI_MakePipe ,std::unique_ptr<BRepOffsetAPI_MakePipe>  , BRepPrimAPI_MakeSweep >(m,"BRepOffsetAPI_MakePipe",R"#(Describes functions to build pipes. A pipe is built a basis shape (called the profile) along a wire (called the spine) by sweeping. The profile must not contain solids. A MakePipe object provides a framework for: - defining the construction of a pipe, - implementing the construction algorithm, and - consulting the result. Warning The MakePipe class implements pipe constructions with G1 continuous spines only.)#");
    ...
    py::class_<BRepOffsetAPI_FindContigousEdges ,std::unique_ptr<BRepOffsetAPI_FindContigousEdges>  >(m,"BRepOffsetAPI_FindContigousEdges",R"#(Provides methods to identify contigous boundaries for continuity control (C0, C1, ...))#");

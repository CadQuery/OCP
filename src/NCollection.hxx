...
void preregister_template_NCollection_UtfString(py::object &m, const char *name){
    py::class_<NCollection_UtfString<Type> ,std::unique_ptr<NCollection_UtfString<Type>> >(m,name,R"#(This template class represent constant UTF-* string. String stored in memory continuously, always NULL-terminated and can be used as standard C-string using ToCString() method.)#");
}

template <typename Type>
void register_template_NCollection_UtfString(py::object &m, const char *name){
    static_cast<py::class_<NCollection_UtfString<Type> ,std::unique_ptr<NCollection_UtfString<Type>> >>(m.attr(name))
        .def(py::init<  >()  )
...
void preregister_template_NCollection_Map(py::object &m, const char *name){
    py::class_<NCollection_Map<TheKeyType,Hasher> ,std::unique_ptr<NCollection_Map<TheKeyType,Hasher>> >(m,name,R"#(Purpose: Single hashed Map. This Map is used to store and retrieve keys in linear time.)#");
}

template <typename TheKeyType,typename Hasher=NCollection_DefaultHasher<TheKeyType>>
void register_template_NCollection_Map(py::object &m, const char *name){
    static_cast<py::class_<NCollection_Map<TheKeyType,Hasher> ,std::unique_ptr<NCollection_Map<TheKeyType,Hasher>> >>(m.attr(name))
        .def(py::init<  >()  )
...
void register_template_NCollection_StlIterator(py::object &m, const char *name){
    static_cast<py::class_<NCollection_StlIterator<Category,BaseIterator,ItemType,IsConstant> ,std::unique_ptr<NCollection_StlIterator<Category,BaseIterator,ItemType,IsConstant>> >>(m.attr(name))
        .def(py::init<  >()  )
...
void preregister_template_NCollection_EBTree(py::object &m, const char *name){
    py::class_<NCollection_EBTree<TheObjType,TheBndType> ,std::unique_ptr<NCollection_EBTree<TheObjType,TheBndType>> >(m,name,R"#(The algorithm of unbalanced binary tree of overlapped bounding boxes with the possibility of deleting objects from the tree.)#");
}

template <typename TheObjType,typename TheBndType>
void register_template_NCollection_EBTree(py::object &m, const char *name){
    static_cast<py::class_<NCollection_EBTree<TheObjType,TheBndType> ,std::unique_ptr<NCollection_EBTree<TheObjType,TheBndType>> >>(m.attr(name))
...
void preregister_template_NCollection_SparseArray(py::object &m, const char *name){
    py::class_<NCollection_SparseArray<TheItemType> ,std::unique_ptr<NCollection_SparseArray<TheItemType>> >(m,name,R"#(Dynamically resizable sparse array of objects)#");
}

template <typename TheItemType>
void register_template_NCollection_SparseArray(py::object &m, const char *name){
    static_cast<py::class_<NCollection_SparseArray<TheItemType> ,std::unique_ptr<NCollection_SparseArray<TheItemType>> >>(m.attr(name))
...
void preregister_template_NCollection_IndexedDataMap(py::object &m, const char *name){
    py::class_<NCollection_IndexedDataMap<TheKeyType,TheItemType,Hasher> ,std::unique_ptr<NCollection_IndexedDataMap<TheKeyType,TheItemType,Hasher>> >(m,name,R"#(Purpose: An indexed map is used to store keys and to bind an index to them. Each new key stored in the map gets an index. Index are incremented as keys are stored in the map. A key can be found by the index and an index by the key. No key but the last can be removed so the indices are in the range 1.. Extent. An Item is stored with each key.)#");
}

template <typename TheKeyType,typename TheItemType,typename Hasher=NCollection_DefaultHasher<TheKeyType>>
void register_template_NCollection_IndexedDataMap(py::object &m, const char *name){
    static_cast<py::class_<NCollection_IndexedDataMap<TheKeyType,TheItemType,Hasher> ,std::unique_ptr<NCollection_IndexedDataMap<TheKeyType,TheItemType,Hasher>> >>(m.attr(name))
        .def(py::init<  >()  )
        .def(py::init< const Standard_Integer,const opencascade::handle<NCollection_BaseAllocator> & >()  , py::arg("theNbBuckets"),  py::arg("theAllocator")=static_cast<const opencascade::handle<NCollection_BaseAllocator> &>(0L) )
...
void preregister_template_NCollection_Mat4(py::object &m, const char *name){
    py::class_<NCollection_Mat4<Element_t> ,std::unique_ptr<NCollection_Mat4<Element_t>> >(m,name,R"#(Generic matrix of 4 x 4 elements. To be used in conjunction with NCollection_Vec4 entities. Originally introduced for 3D space projection and orientation operations.)#");
}

template <typename Element_t>
void register_template_NCollection_Mat4(py::object &m, const char *name){
    static_cast<py::class_<NCollection_Mat4<Element_t> ,std::unique_ptr<NCollection_Mat4<Element_t>> >>(m.attr(name))
        .def(py::init<  >()  )
        .def("GetValue",
...
void preregister_template_NCollection_UtfIterator(py::object &m, const char *name){
    py::class_<NCollection_UtfIterator<Type> ,std::unique_ptr<NCollection_UtfIterator<Type>> >(m,name,R"#(Template class for Unicode strings support.)#");
}

template <typename Type>
void register_template_NCollection_UtfIterator(py::object &m, const char *name){
    static_cast<py::class_<NCollection_UtfIterator<Type> ,std::unique_ptr<NCollection_UtfIterator<Type>> >>(m.attr(name))
...
void preregister_template_NCollection_Vector(py::object &m, const char *name){
    py::class_<NCollection_Vector<TheItemType> ,std::unique_ptr<NCollection_Vector<TheItemType>> >(m,name,R"#(Class NCollection_Vector (dynamic array of objects))#");
}

template <typename TheItemType>
void register_template_NCollection_Vector(py::object &m, const char *name){
    static_cast<py::class_<NCollection_Vector<TheItemType> ,std::unique_ptr<NCollection_Vector<TheItemType>> >>(m.attr(name))
...
void preregister_template_NCollection_Lerp(py::object &m, const char *name){
    py::class_<NCollection_Lerp<T> ,std::unique_ptr<NCollection_Lerp<T>> >(m,name,R"#(Simple linear interpolation tool (also known as mix() in GLSL). The main purpose of this template class is making interpolation routines more readable.)#");
}

template <typename T>
void register_template_NCollection_Lerp(py::object &m, const char *name){
    static_cast<py::class_<NCollection_Lerp<T> ,std::unique_ptr<NCollection_Lerp<T>> >>(m.attr(name))
        .def(py::init<  >()  )
...
void register_template_NCollection_DefaultHasher(py::object &m, const char *name){
    static_cast<py::class_<NCollection_DefaultHasher<TheKeyType> ,std::unique_ptr<NCollection_DefaultHasher<TheKeyType>> >>(m.attr(name))
        .def_static("HashCode_s",
                    ...
                    R"#(None)#"  , py::arg("theKey"),  py::arg("Upper"))
...
void preregister_template_NCollection_IndexedMap(py::object &m, const char *name){
    py::class_<NCollection_IndexedMap<TheKeyType,Hasher> ,std::unique_ptr<NCollection_IndexedMap<TheKeyType,Hasher>> >(m,name,R"#(Purpose: An indexed map is used to store keys and to bind an index to them. Each new key stored in the map gets an index. Index are incremented as keys are stored in the map. A key can be found by the index and an index by the key. No key but the last can be removed so the indices are in the range 1..Extent. See the class Map from NCollection for a discussion about the number of buckets.)#");
}

template <typename TheKeyType,typename Hasher=NCollection_DefaultHasher<TheKeyType>>
void register_template_NCollection_IndexedMap(py::object &m, const char *name){
    static_cast<py::class_<NCollection_IndexedMap<TheKeyType,Hasher> ,std::unique_ptr<NCollection_IndexedMap<TheKeyType,Hasher>> >>(m.attr(name))
        .def(py::init<  >()  )
        .def(py::init< const Standard_Integer,const opencascade::handle<NCollection_BaseAllocator> & >()  , py::arg("theNbBuckets"),  py::arg("theAllocator")=static_cast<const opencascade::handle<NCollection_BaseAllocator> &>(0L) )
...
void preregister_template_NCollection_Handle(py::object &m, const char *name){
    py::class_<NCollection_Handle<T> ,std::unique_ptr<NCollection_Handle<T>> >(m,name,R"#(Purpose: This template class is used to define Handle adaptor for allocated dynamically objects of arbitrary type.)#");
}

template <typename T>
void register_template_NCollection_Handle(py::object &m, const char *name){
    static_cast<py::class_<NCollection_Handle<T> ,std::unique_ptr<NCollection_Handle<T>> >>(m.attr(name))
        .def(py::init<  >()  )
...
void preregister_template_NCollection_TListNode(py::object &m, const char *name){
    py::class_<NCollection_TListNode<TheItemType> ,std::unique_ptr<NCollection_TListNode<TheItemType>> >(m,name,R"#(Purpose: Abstract list node class. Used by BaseList Remark: Internal class)#");
}

template <typename TheItemType>
void register_template_NCollection_TListNode(py::object &m, const char *name){
    static_cast<py::class_<NCollection_TListNode<TheItemType> ,std::unique_ptr<NCollection_TListNode<TheItemType>> >>(m.attr(name))
...
void preregister_template_NCollection_Sequence(py::object &m, const char *name){
    py::class_<NCollection_Sequence<TheItemType> ,std::unique_ptr<NCollection_Sequence<TheItemType>> >(m,name,R"#(Purpose: Definition of a sequence of elements indexed by an Integer in range of 1..n)#");
}

template <typename TheItemType>
void register_template_NCollection_Sequence(py::object &m, const char *name){
    static_cast<py::class_<NCollection_Sequence<TheItemType> ,std::unique_ptr<NCollection_Sequence<TheItemType>> >>(m.attr(name))
        .def(py::init<  >()  )
        ...
        .def("InsertBefore",
             ...
             R"#(InsertBefore theIndex another sequence)#"  , py::arg("theIndex"),  py::arg("theSeq"))
        .def("InsertAfter",
             ...
             R"#(InsertAfter theIndex theItem)#"  , py::arg("theIndex"),  py::arg("theSeq"))
        .def("InsertAfter",
             ...
             R"#(InsertAfter theIndex another sequence)#"  , py::arg("theIndex"),  py::arg("theItem"))
        .def("Split",
...
void preregister_template_NCollection_Vec2(py::object &m, const char *name){
    py::class_<NCollection_Vec2<Element_t> ,std::unique_ptr<NCollection_Vec2<Element_t>> >(m,name,R"#(Defines the 2D-vector template. The main target for this class - to handle raw low-level arrays (from/to graphic driver etc.).)#");
}

template <typename Element_t>
void register_template_NCollection_Vec2(py::object &m, const char *name){
    static_cast<py::class_<NCollection_Vec2<Element_t> ,std::unique_ptr<NCollection_Vec2<Element_t>> >>(m.attr(name))
        .def(py::init<  >()  )
...
void preregister_template_NCollection_Vec3(py::object &m, const char *name){
    py::class_<NCollection_Vec3<Element_t> ,std::unique_ptr<NCollection_Vec3<Element_t>> >(m,name,R"#(Generic 3-components vector. To be used as RGB color pixel or XYZ 3D-point. The main target for this class - to handle raw low-level arrays (from/to graphic driver etc.).)#");
}

template <typename Element_t>
void register_template_NCollection_Vec3(py::object &m, const char *name){
    static_cast<py::class_<NCollection_Vec3<Element_t> ,std::unique_ptr<NCollection_Vec3<Element_t>> >>(m.attr(name))
        .def(py::init<  >()  )
        ...
        .def("b",
             ...
             R"#(Alias to 3rd component as BLUE channel in RGB.)#" )
        .def("xy",
             (NCollection_Vec2<Element_t> & (NCollection_Vec3<Element_t>::*)() ) &NCollection_Vec3<Element_t>::xy,
             R"#(Returns XY-components modifiable vector)#" )
        .def("yz",
             (NCollection_Vec2<Element_t> & (NCollection_Vec3<Element_t>::*)() ) &NCollection_Vec3<Element_t>::yz,
             R"#(Returns YZ-components modifiable vector)#" )
        .def("IsEqual",
...
void preregister_template_NCollection_DataMap(py::object &m, const char *name){
    py::class_<NCollection_DataMap<TheKeyType,TheItemType,Hasher> ,std::unique_ptr<NCollection_DataMap<TheKeyType,TheItemType,Hasher>> >(m,name,R"#(Purpose: The DataMap is a Map to store keys with associated Items. See Map from NCollection for a discussion about the number of buckets.)#");
}

template <typename TheKeyType,typename TheItemType,typename Hasher=NCollection_DefaultHasher<TheKeyType>>
void register_template_NCollection_DataMap(py::object &m, const char *name){
    static_cast<py::class_<NCollection_DataMap<TheKeyType,TheItemType,Hasher> ,std::unique_ptr<NCollection_DataMap<TheKeyType,TheItemType,Hasher>> >>(m.attr(name))
        .def(py::init<  >()  )
        .def(py::init< const Standard_Integer,const opencascade::handle<NCollection_BaseAllocator> & >()  , py::arg("theNbBuckets"),  py::arg("theAllocator")=static_cast<const opencascade::handle<NCollection_BaseAllocator> &>(0L) )
...
void preregister_template_NCollection_DoubleMap(py::object &m, const char *name){
    py::class_<NCollection_DoubleMap<TheKey1Type,TheKey2Type,Hasher1,Hasher2> ,std::unique_ptr<NCollection_DoubleMap<TheKey1Type,TheKey2Type,Hasher1,Hasher2>> >(m,name,R"#(Purpose: The DoubleMap is used to bind pairs (Key1,Key2) and retrieve them in linear time.)#");
}

template <typename TheKey1Type,typename TheKey2Type,typename Hasher1=NCollection_DefaultHasher<TheKey1Type>,typename Hasher2=NCollection_DefaultHasher<TheKey2Type>>
void register_template_NCollection_DoubleMap(py::object &m, const char *name){
    static_cast<py::class_<NCollection_DoubleMap<TheKey1Type,TheKey2Type,Hasher1,Hasher2> ,std::unique_ptr<NCollection_DoubleMap<TheKey1Type,TheKey2Type,Hasher1,Hasher2>> >>(m.attr(name))
        .def(py::init<  >()  )
        .def(py::init< const Standard_Integer,const opencascade::handle<NCollection_BaseAllocator> & >()  , py::arg("theNbBuckets"),  py::arg("theAllocator")=static_cast<const opencascade::handle<NCollection_BaseAllocator> &>(0L) )
...
void register_template_NCollection_UBTreeFiller(py::object &m, const char *name){
    static_cast<py::class_<NCollection_UBTreeFiller<TheObjType,TheBndType> ,std::unique_ptr<NCollection_UBTreeFiller<TheObjType,TheBndType>> >>(m.attr(name))
        ...
        .def("CheckTree",
             ...
             R"#(Check the filled tree for the total number of items and the balance outputting these results to ostream.)#"  , py::arg("theStream"))
    ;
};
...
void preregister_template_NCollection_LocalArray(py::object &m, const char *name){
    py::class_<NCollection_LocalArray<theItem,MAX_ARRAY_SIZE> ,std::unique_ptr<NCollection_LocalArray<theItem,MAX_ARRAY_SIZE>> >(m,name,R"#(Auxiliary class optimizing creation of array buffer (using stack allocation for small arrays).)#");
}

template <typename theItem,Standard_Integer MAX_ARRAY_SIZE=1024>
void register_template_NCollection_LocalArray(py::object &m, const char *name){
    static_cast<py::class_<NCollection_LocalArray<theItem,MAX_ARRAY_SIZE> ,std::unique_ptr<NCollection_LocalArray<theItem,MAX_ARRAY_SIZE>> >>(m.attr(name))
...
void preregister_template_NCollection_List(py::object &m, const char *name){
    py::class_<NCollection_List<TheItemType> ,std::unique_ptr<NCollection_List<TheItemType>> >(m,name,R"#(Purpose: Simple list to link items together keeping the first and the last one. Inherits BaseList, adding the data item to each node.)#");
}

template <typename TheItemType>
void register_template_NCollection_List(py::object &m, const char *name){
    static_cast<py::class_<NCollection_List<TheItemType> ,std::unique_ptr<NCollection_List<TheItemType>> >>(m.attr(name))
        .def(py::init<  >()  )
        .def(py::init< const opencascade::handle<NCollection_BaseAllocator> & >()  , py::arg("theAllocator") )
        ...
        .def("Append",
             ...
             R"#(Append another list at the end)#"  , py::arg("theOther"))
        .def("Prepend",
...
void preregister_template_NCollection_Array2(py::object &m, const char *name){
    py::class_<NCollection_Array2<TheItemType> ,std::unique_ptr<NCollection_Array2<TheItemType>> >(m,name,R"#(Purpose: The class Array2 represents bi-dimensional arrays of fixed size known at run time. The ranges of indices are user defined.)#");
}

template <typename TheItemType>
void register_template_NCollection_Array2(py::object &m, const char *name){
    static_cast<py::class_<NCollection_Array2<TheItemType> ,std::unique_ptr<NCollection_Array2<TheItemType>> >>(m.attr(name))
...
void register_template_NCollection_Vec4(py::object &m, const char *name){
    static_cast<py::class_<NCollection_Vec4<Element_t> ,std::unique_ptr<NCollection_Vec4<Element_t>> >>(m.attr(name))
        .def(py::init<  >()  )
        ...
        .def(py::init< const NCollection_Vec2<Element_t> & >()  , py::arg("theVec2") )
        .def(py::init< const NCollection_Vec3<Element_t> & >()  , py::arg("theVec3") )
        .def(py::init< const NCollection_Vec3<Element_t> &,const Element_t >()  , py::arg("theVec3"),  py::arg("theAlpha") )
        .def("SetValues",
        ...
        .def("a",
             ...
             R"#(Alias to 4th component as ALPHA channel in RGBA.)#" )
        .def("xy",
             (NCollection_Vec2<Element_t> & (NCollection_Vec4<Element_t>::*)() ) &NCollection_Vec4<Element_t>::xy,
             R"#(Returns XY-components modifiable vector)#" )
        .def("yz",
             (NCollection_Vec2<Element_t> & (NCollection_Vec4<Element_t>::*)() ) &NCollection_Vec4<Element_t>::yz,
             R"#(Returns YZ-components modifiable vector)#" )
        .def("zw",
             (NCollection_Vec2<Element_t> & (NCollection_Vec4<Element_t>::*)() ) &NCollection_Vec4<Element_t>::zw,
             R"#(Returns YZ-components modifiable vector)#" )
        .def("xyz",
             (NCollection_Vec3<Element_t> & (NCollection_Vec4<Element_t>::*)() ) &NCollection_Vec4<Element_t>::xyz,
             R"#(Returns XYZ-components modifiable vector)#" )
        .def("yzw",
             (NCollection_Vec3<Element_t> & (NCollection_Vec4<Element_t>::*)() ) &NCollection_Vec4<Element_t>::yzw,
             R"#(Returns YZW-components modifiable vector)#" )
        .def("IsEqual",
...
void preregister_template_NCollection_UBTree(py::object &m, const char *name){
    py::class_<NCollection_UBTree<TheObjType,TheBndType> ,std::unique_ptr<NCollection_UBTree<TheObjType,TheBndType>> >(m,name,R"#(The algorithm of unbalanced binary tree of overlapped bounding boxes.)#");
}

template <typename TheObjType,typename TheBndType>
void register_template_NCollection_UBTree(py::object &m, const char *name){
    static_cast<py::class_<NCollection_UBTree<TheObjType,TheBndType> ,std::unique_ptr<NCollection_UBTree<TheObjType,TheBndType>> >>(m.attr(name))
        .def(py::init<  >()  )
        .def(py::init< const opencascade::handle<NCollection_BaseAllocator> & >()  , py::arg("theAllocator") )
        .def("Add",
...
void preregister_template_NCollection_TListIterator(py::object &m, const char *name){
    py::class_<NCollection_TListIterator<TheItemType> ,std::unique_ptr<NCollection_TListIterator<TheItemType>> >(m,name,R"#(Purpose: This Iterator class iterates on BaseList of TListNode and is instantiated in List/Set/Queue/Stack Remark: TListIterator is internal class)#");
}

template <typename TheItemType>
void register_template_NCollection_TListIterator(py::object &m, const char *name){
    static_cast<py::class_<NCollection_TListIterator<TheItemType> ,std::unique_ptr<NCollection_TListIterator<TheItemType>> >>(m.attr(name))
        .def(py::init<  >()  )

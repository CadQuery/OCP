...
// classes forward declarations only
    py::class_<Approx_HArray1OfAdHSurface ,std::unique_ptr<Approx_HArray1OfAdHSurface>  >(m,"Approx_HArray1OfAdHSurface",R"#()#");
    py::class_<Approx_CurvilinearParameter ,std::unique_ptr<Approx_CurvilinearParameter>  >(m,"Approx_CurvilinearParameter",R"#(Approximation of a Curve to make its parameter be its curvilinear abscissa If the curve is a curve on a surface S, C2D is the corresponding Pcurve, we considere the curve is given by its representation S(C2D(u)) If the curve is a curve on 2 surfaces S1 and S2 and C2D1 C2D2 are the two corresponding Pcurve, we considere the curve is given by its representation 1/2(S1(C2D1(u) + S2 (C2D2(u))))#");
    py::class_<Approx_MCurvesToBSpCurve ,std::unique_ptr<Approx_MCurvesToBSpCurve>  >(m,"Approx_MCurvesToBSpCurve",R"#(None)#");
    py::class_<Approx_Curve2d ,std::unique_ptr<Approx_Curve2d>  >(m,"Approx_Curve2d",R"#(Makes an approximation for HCurve2d from Adaptor3d)#");
    py::class_<Approx_FitAndDivide2d ,std::unique_ptr<Approx_FitAndDivide2d>  >(m,"Approx_FitAndDivide2d",R"#(None)#");
    py::class_<Approx_CurveOnSurface ,std::unique_ptr<Approx_CurveOnSurface>  >(m,"Approx_CurveOnSurface",R"#(Approximation of curve on surface)#");
    ...
    py::class_<Approx_Curve3d ,std::unique_ptr<Approx_Curve3d>  >(m,"Approx_Curve3d",R"#(None)#");
    py::class_<Approx_FitAndDivide ,std::unique_ptr<Approx_FitAndDivide>  >(m,"Approx_FitAndDivide",R"#(None)#");
    py::class_<Approx_HArray1OfGTrsf2d ,std::unique_ptr<Approx_HArray1OfGTrsf2d>  >(m,"Approx_HArray1OfGTrsf2d",R"#()#");
    py::class_<Approx_SweepApproximation ,std::unique_ptr<Approx_SweepApproximation>  >(m,"Approx_SweepApproximation",R"#(Approximation of an Surface S(u,v) (and eventually associate 2d Curves) defined by section's law.)#");
    py::class_<Approx_SameParameter ,std::unique_ptr<Approx_SameParameter>  >(m,"Approx_SameParameter",R"#(Approximation of a PCurve on a surface to make its parameter be the same that the parameter of a given 3d reference curve.)#");

...
// classes forward declarations only
    py::class_<math_Function ,std::unique_ptr<math_Function> ,Py_math_Function >(m,"math_Function",R"#(This abstract class describes the virtual functions associated with a Function of a single variable.)#");
    py::class_<math_MultipleVarFunction ,std::unique_ptr<math_MultipleVarFunction> ,Py_math_MultipleVarFunction >(m,"math_MultipleVarFunction",R"#(Describes the virtual functions associated with a multiple variable function.)#");
    py::class_<math_FunctionSet ,std::unique_ptr<math_FunctionSet> ,Py_math_FunctionSet >(m,"math_FunctionSet",R"#(This abstract class describes the virtual functions associated to a set on N Functions of M independant variables.)#");
    py::class_<math_ValueAndWeight ,std::unique_ptr<math_ValueAndWeight>  >(m,"math_ValueAndWeight",R"#(Simple container storing two reals: value and weight)#");
    py::class_<math_GaussMultipleIntegration ,std::unique_ptr<math_GaussMultipleIntegration>  >(m,"math_GaussMultipleIntegration",R"#(This class implements the integration of a function of multiple variables between the parameter bounds Lower[a..b] and Upper[a..b]. Warning: Each element of Order must be inferior or equal to 61.)#");
    py::class_<math_BracketedRoot ,std::unique_ptr<math_BracketedRoot>  >(m,"math_BracketedRoot",R"#(This class implements the Brent method to find the root of a function located within two bounds. No knowledge of the derivative is required.)#");
    py::class_<math_FunctionRoot ,std::unique_ptr<math_FunctionRoot>  >(m,"math_FunctionRoot",R"#(This class implements the computation of a root of a function of a single variable which is near an initial guess using a minimization algorithm.Knowledge of the derivative is required. The algorithm used is the same as in)#");
    py::class_<math ,std::unique_ptr<math>  >(m,"math",R"#(None)#");
    py::class_<math_FRPR ,std::unique_ptr<math_FRPR>  >(m,"math_FRPR",R"#(this class implements the Fletcher-Reeves-Polak_Ribiere minimization algorithm of a function of multiple variables. Knowledge of the function's gradient is required.)#");
    py::class_<math_Powell ,std::unique_ptr<math_Powell>  >(m,"math_Powell",R"#(This class implements the Powell method to find the minimum of function of multiple variables (the gradient does not have to be known).)#");
    py::class_<math_FunctionSample ,std::unique_ptr<math_FunctionSample>  >(m,"math_FunctionSample",R"#(This class gives a default sample (constant difference of parameter) for a function defined between two bound A,B.)#");
    py::class_<math_Uzawa ,std::unique_ptr<math_Uzawa>  >(m,"math_Uzawa",R"#(This class implements a system resolution C*X = B with an approach solution X0. There are no conditions on the number of equations. The algorithm used is the Uzawa algorithm. It is possible to have equal or inequal (<) equations to solve. The resolution is done with a minimization of Norm(X-X0). If there are only equal equations, the resolution is directly done and is similar to Gauss resolution with an optimisation because the matrix is a symmetric matrix. (The resolution is done with Crout algorithm))#");
    py::class_<math_TrigonometricFunctionRoots ,std::unique_ptr<math_TrigonometricFunctionRoots>  >(m,"math_TrigonometricFunctionRoots",R"#(This class implements the solutions of the equation a*Cos(x)*Cos(x) + 2*b*Cos(x)*Sin(x) + c*Cos(x) + d*Sin(x) + e The degree of this equation can be 4, 3 or 2.)#");
    py::class_<math_GaussSetIntegration ,std::unique_ptr<math_GaussSetIntegration>  >(m,"math_GaussSetIntegration",R"#(-- This class implements the integration of a set of N functions of M variables variables between the parameter bounds Lower[a..b] and Upper[a..b]. Warning: - The case M>1 is not implemented.)#");
    py::class_<math_MultipleVarFunctionWithGradient ,std::unique_ptr<math_MultipleVarFunctionWithGradient> ,Py_math_MultipleVarFunctionWithGradient , math_MultipleVarFunction >(m,"math_MultipleVarFunctionWithGradient",R"#(The abstract class MultipleVarFunctionWithGradient describes the virtual functions associated with a multiple variable function.)#");
    py::class_<math_FunctionWithDerivative ,std::unique_ptr<math_FunctionWithDerivative> ,Py_math_FunctionWithDerivative , math_Function >(m,"math_FunctionWithDerivative",R"#(This abstract class describes the virtual functions associated with a function of a single variable for which the first derivative is available.)#");
    py::class_<math_GaussSingleIntegration ,std::unique_ptr<math_GaussSingleIntegration>  >(m,"math_GaussSingleIntegration",R"#(This class implements the integration of a function of a single variable between the parameter bounds Lower and Upper. Warning: Order must be inferior or equal to 61.)#");
    py::class_<math_BissecNewton ,std::unique_ptr<math_BissecNewton>  >(m,"math_BissecNewton",R"#(This class implements a combination of Newton-Raphson and bissection methods to find the root of the function between two bounds. Knowledge of the derivative is required.)#");
    py::class_<math_Householder ,std::unique_ptr<math_Householder>  >(m,"math_Householder",R"#(This class implements the least square solution of a set of linear equations of m unknowns (n >= m) using the Householder method. It solves A.X = B. This algorithm has more numerical stability than GaussLeastSquare but is longer. It must be used if the matrix is singular or nearly singular. It is about 16% longer than GaussLeastSquare if there is only one member B to solve. It is about 30% longer if there are twenty B members to solve.)#");
    py::class_<math_Vector ,std::unique_ptr<math_Vector>  >(m,"math_Vector",R"#(This class implements the real vector abstract data type. Vectors can have an arbitrary range which must be defined at the declaration and cannot be changed after this declaration.)#");
    py::class_<math_Jacobi ,std::unique_ptr<math_Jacobi>  >(m,"math_Jacobi",R"#(This class implements the Jacobi method to find the eigenvalues and the eigenvectors of a real symmetric square matrix. A sort of eigenvalues is done.)#");
    py::class_<math_NewtonFunctionRoot ,std::unique_ptr<math_NewtonFunctionRoot>  >(m,"math_NewtonFunctionRoot",R"#(This class implements the calculation of a root of a function of a single variable starting from an initial near guess using the Newton algorithm. Knowledge of the derivative is required.)#");
    py::class_<math_GlobOptMin ,std::unique_ptr<math_GlobOptMin>  >(m,"math_GlobOptMin",R"#(This class represents Evtushenko's algorithm of global optimization based on non-uniform mesh. Article: Yu. Evtushenko. Numerical methods for finding global extreme (case of a non-uniform mesh). U.S.S.R. Comput. Maths. Math. Phys., Vol. 11, N 6, pp. 38-54.)#");
    py::class_<math_BrentMinimum ,std::unique_ptr<math_BrentMinimum>  >(m,"math_BrentMinimum",R"#(This class implements the Brent's method to find the minimum of a function of a single variable. No knowledge of the derivative is required.)#");
    py::class_<math_Matrix ,std::unique_ptr<math_Matrix>  >(m,"math_Matrix",R"#(This class implements the real matrix abstract data type. Matrixes can have an arbitrary range which must be defined at the declaration and cannot be changed after this declaration math_Matrix(-3,5,2,4); //a vector with range [-3..5, 2..4] Matrix values may be initialized and retrieved using indexes which must lie within the range of definition of the matrix. Matrix objects follow "value semantics", that is, they cannot be shared and are copied through assignment Matrices are copied through assignement: math_Matrix M2(1, 9, 1, 3); ... M2 = M1; M1(1) = 2.0;//the matrix M2 will not be modified.)#");
    py::class_<math_SVD ,std::unique_ptr<math_SVD>  >(m,"math_SVD",R"#(SVD implements the solution of a set of N linear equations of M unknowns without condition on N or M. The Singular Value Decomposition algorithm is used. For singular or nearly singular matrices SVD is a better choice than Gauss or GaussLeastSquare.)#");
    py::class_<math_Crout ,std::unique_ptr<math_Crout>  >(m,"math_Crout",R"#(This class implements the Crout algorithm used to solve a system A*X = B where A is a symmetric matrix. It can be used to invert a symmetric matrix. This algorithm is similar to Gauss but is faster than Gauss. Only the inferior triangle of A and the diagonal can be given.)#");
    py::class_<math_ComputeKronrodPointsAndWeights ,std::unique_ptr<math_ComputeKronrodPointsAndWeights>  >(m,"math_ComputeKronrodPointsAndWeights",R"#(None)#");
    py::class_<math_NewtonMinimum ,std::unique_ptr<math_NewtonMinimum>  >(m,"math_NewtonMinimum",R"#(None)#");
    py::class_<math_FunctionRoots ,std::unique_ptr<math_FunctionRoots>  >(m,"math_FunctionRoots",R"#(This class implements an algorithm which finds all the real roots of a function with derivative within a given range. Knowledge of the derivative is required.)#");
    py::class_<math_ComputeGaussPointsAndWeights ,std::unique_ptr<math_ComputeGaussPointsAndWeights>  >(m,"math_ComputeGaussPointsAndWeights",R"#(None)#");
    py::class_<math_DirectPolynomialRoots ,std::unique_ptr<math_DirectPolynomialRoots>  >(m,"math_DirectPolynomialRoots",R"#(This class implements the calculation of all the real roots of a real polynomial of degree <= 4 using a direct method. Once found, the roots are polished using the Newton method.)#");
    py::class_<math_BullardGenerator ,std::unique_ptr<math_BullardGenerator>  >(m,"math_BullardGenerator",R"#(Fast random number generator (the algorithm proposed by Ian C. Bullard).)#");
    py::class_<math_FunctionAllRoots ,std::unique_ptr<math_FunctionAllRoots>  >(m,"math_FunctionAllRoots",R"#(This algorithm uses a sample of the function to find all intervals on which the function is null, and afterwards uses the FunctionRoots algorithm to find the points where the function is null outside the "null intervals". Knowledge of the derivative is required.)#");
    py::class_<math_KronrodSingleIntegration ,std::unique_ptr<math_KronrodSingleIntegration>  >(m,"math_KronrodSingleIntegration",R"#(This class implements the Gauss-Kronrod method of integral computation.)#");
    py::class_<math_FunctionSetRoot ,std::unique_ptr<math_FunctionSetRoot>  >(m,"math_FunctionSetRoot",R"#(The math_FunctionSetRoot class calculates the root of a set of N functions of M variables (N<M, N=M or N>M). Knowing an initial guess of the solution and using a minimization algorithm, a search is made in the Newton direction and then in the Gradient direction if there is no success in the Newton direction. This algorithm can also be used for functions minimization. Knowledge of all the partial derivatives (the Jacobian) is required.)#");
    py::class_<math_FunctionSetWithDerivatives ,std::unique_ptr<math_FunctionSetWithDerivatives> ,Py_math_FunctionSetWithDerivatives , math_FunctionSet >(m,"math_FunctionSetWithDerivatives",R"#(This abstract class describes the virtual functions associated with a set of N Functions each of M independant variables.)#");
    py::class_<math_EigenValuesSearcher ,std::unique_ptr<math_EigenValuesSearcher>  >(m,"math_EigenValuesSearcher",R"#(This class finds eigen values and vectors of real symmetric tridiagonal matrix)#");
    py::class_<math_Gauss ,std::unique_ptr<math_Gauss>  >(m,"math_Gauss",R"#(This class implements the Gauss LU decomposition (Crout algorithm) with partial pivoting (rows interchange) of a square matrix and the different possible derived calculation : - solution of a set of linear equations. - inverse of a matrix. - determinant of a matrix.)#");
    py::class_<math_MultipleVarFunctionWithHessian ,std::unique_ptr<math_MultipleVarFunctionWithHessian> ,Py_math_MultipleVarFunctionWithHessian , math_MultipleVarFunctionWithGradient >(m,"math_MultipleVarFunctionWithHessian",R"#(None)#");
    py::class_<math_BracketMinimum ,std::unique_ptr<math_BracketMinimum>  >(m,"math_BracketMinimum",R"#(Given two distinct initial points, BracketMinimum implements the computation of three points (a, b, c) which bracket the minimum of the function and verify A less than B, B less than C and F(B) less than F(A), F(B) less than F(C).)#");
    py::class_<math_NewtonFunctionSetRoot ,std::unique_ptr<math_NewtonFunctionSetRoot>  >(m,"math_NewtonFunctionSetRoot",R"#(This class computes the root of a set of N functions of N variables, knowing an initial guess at the solution and using the Newton Raphson algorithm. Knowledge of all the partial derivatives (Jacobian) is required.)#");
    py::class_<math_GaussLeastSquare ,std::unique_ptr<math_GaussLeastSquare>  >(m,"math_GaussLeastSquare",R"#(This class implements the least square solution of a set of n linear equations of m unknowns (n >= m) using the gauss LU decomposition algorithm. This algorithm is more likely subject to numerical instability than math_SVD.)#");
    py::class_<math_IntegerVector ,std::unique_ptr<math_IntegerVector>  >(m,"math_IntegerVector",R"#(This class implements the real IntegerVector abstract data type. IntegerVectors can have an arbitrary range which must be define at the declaration and cannot be changed after this declaration. Example:)#");
    py::class_<math_TrigonometricEquationFunction ,std::unique_ptr<math_TrigonometricEquationFunction>  , math_FunctionWithDerivative >(m,"math_TrigonometricEquationFunction",R"#(This is function, which corresponds trigonometric equation a*Cos(x)*Cos(x) + 2*b*Cos(x)*Sin(x) + c*Cos(x) + d*Sin(x) + e = 0 See class math_TrigonometricFunctionRoots)#");
    py::class_<math_PSO ,std::unique_ptr<math_PSO>  >(m,"math_PSO",R"#(In this class implemented variation of Particle Swarm Optimization (PSO) method. A. Ismael F. Vaz, L. N. Vicente "A particle swarm pattern search method for bound constrained global optimization")#");
    py::class_<math_DoubleTab ,std::unique_ptr<math_DoubleTab>  >(m,"math_DoubleTab",R"#(None)#");
    py::class_<math_PSOParticlesPool ,std::unique_ptr<math_PSOParticlesPool>  >(m,"math_PSOParticlesPool",R"#(None)#");
    py::class_<math_BFGS ,std::unique_ptr<math_BFGS>  >(m,"math_BFGS",R"#(This class implements the Broyden-Fletcher-Goldfarb-Shanno variant of Davidson-Fletcher-Powell minimization algorithm of a function of multiple variables.Knowledge of the function's gradient is required.)#");

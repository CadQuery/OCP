...
// classes forward declarations only
    ...
    py::class_<ShapeFix_EdgeConnect ,std::unique_ptr<ShapeFix_EdgeConnect>  >(m,"ShapeFix_EdgeConnect",R"#(Rebuilds edges to connect with new vertices, was moved from ShapeBuild. Makes vertices to be shared to connect edges, updates positions and tolerances for shared vertices. Accepts edges bounded by two vertices each.)#");
    py::class_<ShapeFix_WireSegment ,std::unique_ptr<ShapeFix_WireSegment>  >(m,"ShapeFix_WireSegment",R"#(This class is auxiliary class (data storage) used in ComposeShell. It is intended for representing segment of the wire (or whole wire). The segment itself is represented by ShapeExtend_WireData. In addition, some associated data necessary for computations are stored:)#");
    ...
    py::class_<ShapeFix_SplitTool ,std::unique_ptr<ShapeFix_SplitTool>  >(m,"ShapeFix_SplitTool",R"#(Tool for splitting and cutting edges; includes methods used in OverlappingTool and IntersectionTool)#");
    py::class_<ShapeFix ,std::unique_ptr<ShapeFix>  >(m,"ShapeFix",R"#(This package provides algorithms for fixing problematic (violating Open CASCADE requirements) shapes. Tools from package ShapeAnalysis are used for detecting the problems. The detecting and fixing is done taking in account various criteria implemented in BRepCheck package. Each class of package ShapeFix deals with one certain type of shapes or with some family of problems.)#");
    py::class_<ShapeFix_ShapeTolerance ,std::unique_ptr<ShapeFix_ShapeTolerance>  >(m,"ShapeFix_ShapeTolerance",R"#(Modifies tolerances of sub-shapes (vertices, edges, faces))#");
    py::class_<ShapeFix_FaceConnect ,std::unique_ptr<ShapeFix_FaceConnect>  >(m,"ShapeFix_FaceConnect",R"#(Rebuilds connectivity between faces in shell)#");

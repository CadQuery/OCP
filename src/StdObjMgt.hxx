...
void preregister_template_StdObjMgt_Attribute(py::object &m, const char *name){
    py::class_<StdObjMgt_Attribute<Transient> ,std::unique_ptr<StdObjMgt_Attribute<Transient>> >(m,name,R"#(Root class for a temporary persistent object corresponding to an attribute.)#");
}

template <typename Transient>
void register_template_StdObjMgt_Attribute(py::object &m, const char *name){
    static_cast<py::class_<StdObjMgt_Attribute<Transient> ,std::unique_ptr<StdObjMgt_Attribute<Transient>> >>(m.attr(name))
    ;
};

...
// classes forward declarations only
    py::class_<BRepLib_Command ,std::unique_ptr<BRepLib_Command>  >(m,"BRepLib_Command",R"#(Root class for all commands in BRepLib.)#");
    py::class_<BRepLib_MakeShape ,std::unique_ptr<BRepLib_MakeShape>  , BRepLib_Command >(m,"BRepLib_MakeShape",R"#(This is the root class for all shape constructions. It stores the result.)#");
    py::class_<BRepLib_MakeEdge ,std::unique_ptr<BRepLib_MakeEdge>  , BRepLib_MakeShape >(m,"BRepLib_MakeEdge",R"#(Provides methods to build edges.)#");
    py::class_<BRepLib_MakeWire ,std::unique_ptr<BRepLib_MakeWire>  , BRepLib_MakeShape >(m,"BRepLib_MakeWire",R"#(Provides methods to build wires.)#");
    py::class_<BRepLib_MakeVertex ,std::unique_ptr<BRepLib_MakeVertex>  , BRepLib_MakeShape >(m,"BRepLib_MakeVertex",R"#(Provides methods to build vertices.)#");
    py::class_<BRepLib_MakeFace ,std::unique_ptr<BRepLib_MakeFace>  , BRepLib_MakeShape >(m,"BRepLib_MakeFace",R"#(Provides methods to build faces.)#");
    py::class_<BRepLib_FuseEdges ,std::unique_ptr<BRepLib_FuseEdges>  >(m,"BRepLib_FuseEdges",R"#(This class can detect vertices in a face that can be considered useless and then perform the fuse of the edges and remove the useless vertices. By useles vertices, we mean : * vertices that have exactly two connex edges * the edges connex to the vertex must have exactly the same 2 connex faces . * The edges connex to the vertex must have the same geometric support.)#");
    py::class_<BRepLib_MakeShell ,std::unique_ptr<BRepLib_MakeShell>  , BRepLib_MakeShape >(m,"BRepLib_MakeShell",R"#(Provides methos to build shells.)#");
    py::class_<BRepLib_MakeSolid ,std::unique_ptr<BRepLib_MakeSolid>  , BRepLib_MakeShape >(m,"BRepLib_MakeSolid",R"#(Makes a solid from compsolid or shells.)#");
    py::class_<BRepLib ,std::unique_ptr<BRepLib>  >(m,"BRepLib",R"#(The BRepLib package provides general utilities for BRep.)#");
    py::class_<BRepLib_FindSurface ,std::unique_ptr<BRepLib_FindSurface>  >(m,"BRepLib_FindSurface",R"#(Provides an algorithm to find a Surface through a set of edges.)#");
    py::class_<BRepLib_MakeEdge2d ,std::unique_ptr<BRepLib_MakeEdge2d>  , BRepLib_MakeShape >(m,"BRepLib_MakeEdge2d",R"#(Provides methods to build edges.)#");
    py::class_<BRepLib_MakePolygon ,std::unique_ptr<BRepLib_MakePolygon>  , BRepLib_MakeShape >(m,"BRepLib_MakePolygon",R"#(Class to build polygonal wires.)#");

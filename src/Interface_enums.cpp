...
// classes forward declarations only
    py::class_<Interface_EntityIterator ,std::unique_ptr<Interface_EntityIterator>  >(m,"Interface_EntityIterator",R"#(Defines an Iterator on Entities. Allows considering of various criteria)#");
    py::class_<Interface_CheckTool ,std::unique_ptr<Interface_CheckTool>  >(m,"Interface_CheckTool",R"#(Performs Checks on Entities, using General Service Library and Modules to work. Works on one Entity or on a complete Model)#");
    ...
    py::class_<Interface_ShareTool ,std::unique_ptr<Interface_ShareTool>  >(m,"Interface_ShareTool",R"#(Builds the Graph of Dependancies, from the General Service "Shared" -> builds for each Entity of a Model, the Shared and Sharing Lists, and gives access to them. Allows to complete with Implied References (which are not regarded as Shared Entities, but are nevertheless Referenced), this can be usefull for Reference Checking)#");
    py::class_<Interface_LineBuffer ,std::unique_ptr<Interface_LineBuffer>  >(m,"Interface_LineBuffer",R"#(Simple Management of a Line Buffer, to be used by Interface File Writers. While a String is suitable to do that, this class ensures an optimised Memory Management, because this is a hard point of File Writing.)#");
    ...
    py::class_<Interface_HArray1OfHAsciiString ,std::unique_ptr<Interface_HArray1OfHAsciiString>  >(m,"Interface_HArray1OfHAsciiString",R"#()#");
    ...
    py::class_<Interface_FileReaderTool ,std::unique_ptr<Interface_FileReaderTool> ,Py_Interface_FileReaderTool >(m,"Interface_FileReaderTool",R"#(Defines services which are required to load an InterfaceModel from a File. Typically, it may firstly transform a system file into a FileReaderData object, then work on it, not longer considering file contents, to load an Interface Model. It may also work on a FileReaderData already loaded.)#");
    py::class_<Interface_BitMap ,std::unique_ptr<Interface_BitMap>  >(m,"Interface_BitMap",R"#(A bit map simply allows to associate a boolean flag to each item of a list, such as a list of entities, etc... numbered between 1 and a positive count nbitems)#");
    ...
    py::class_<Interface_CopyTool ,std::unique_ptr<Interface_CopyTool>  >(m,"Interface_CopyTool",R"#(Performs Deep Copies of sets of Entities Allows to perform Copy of Interface Entities from a Model to another one. Works by calling general services GetFromAnother and GetImplied. Uses a CopyMap to bind a unique Result to each Copied Entity)#");
    py::class_<Interface_ShareFlags ,std::unique_ptr<Interface_ShareFlags>  >(m,"Interface_ShareFlags",R"#(This class only says for each Entity of a Model, if it is Shared or not by one or more other(s) of this Model It uses the General Service "Shared".)#");
    py::class_<Interface_ParamList ,opencascade::handle<Interface_ParamList>  , Standard_Transient >(m,"Interface_ParamList",R"#()#");
    py::class_<Interface_HSequenceOfCheck ,std::unique_ptr<Interface_HSequenceOfCheck>  >(m,"Interface_HSequenceOfCheck",R"#()#");
    ...
    py::class_<Interface_FileParameter ,std::unique_ptr<Interface_FileParameter>  >(m,"Interface_FileParameter",R"#(Auxiliary class to store a litteral parameter in a file intermediate directory or in an UndefinedContent : a reference type Parameter detains an Integer which is used to address a record in the directory. FileParameter is intended to be stored in a ParamSet : hence memory management is performed by ParamSet, which calls Clear to work, while the Destructor (see Destroy) does nothing. Also a FileParameter can be read for consultation only, not to be read from a Structure to be included into another one.)#");
    py::class_<Interface_GeneralLib ,std::unique_ptr<Interface_GeneralLib>  >(m,"Interface_GeneralLib",R"#(None)#");
    py::class_<Interface_MSG ,std::unique_ptr<Interface_MSG>  >(m,"Interface_MSG",R"#(This class gives a set of functions to manage and use a list of translated messages (messagery))#");
    py::class_<Interface_MapAsciiStringHasher ,std::unique_ptr<Interface_MapAsciiStringHasher>  >(m,"Interface_MapAsciiStringHasher",R"#(None)#");
    py::class_<Interface_ReaderLib ,std::unique_ptr<Interface_ReaderLib>  >(m,"Interface_ReaderLib",R"#(None)#");
    ...
    py::class_<Interface_Graph ,std::unique_ptr<Interface_Graph>  >(m,"Interface_Graph",R"#(Gives basic data structure for operating and storing graph results (usage is normally internal) Entities are Mapped according their Number in the Model)#");
    py::class_<Interface_Category ,std::unique_ptr<Interface_Category>  >(m,"Interface_Category",R"#(This class manages categories A category is defined by a name and a number, and can be seen as a way of rough classification, i.e. less precise than a cdl type. Hence, it is possible to dispatch every entity in about a dozen of categories, twenty is a reasonable maximum.)#");
    ...
    py::class_<Interface_GraphContent ,std::unique_ptr<Interface_GraphContent>  , Interface_EntityIterator >(m,"Interface_GraphContent",R"#(Defines general form for classes of graph algorithms on Interfaces, this form is that of EntityIterator Each sub-class fills it according to its own algorithm This also allows to combine any graph result to others, all being given under one unique form)#");
    ...
    py::class_<Interface_FloatWriter ,std::unique_ptr<Interface_FloatWriter>  >(m,"Interface_FloatWriter",R"#(This class converts a floting number (Real) to a string It can be used if the standard C-C++ output functions (sprintf or cout<<) are not convenient. That is to say : - to suppress trailing '0' and 'E+00' (if desired) - to control exponant output and floating point output)#");

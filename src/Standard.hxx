...
void preregister_template_Standard_Static_Assert(py::object &m, const char *name){
    py::class_<Standard_Static_Assert<condition> ,std::unique_ptr<Standard_Static_Assert<condition>> >(m,name,R"#(Static assert -- empty default template)#");
}

template <bool condition>
void register_template_Standard_Static_Assert(py::object &m, const char *name){
    static_cast<py::class_<Standard_Static_Assert<condition> ,std::unique_ptr<Standard_Static_Assert<condition>> >>(m.attr(name))
    ;
};

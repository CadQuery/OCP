...
// classes forward declarations only
    py::class_<StepData_FieldList ,std::unique_ptr<StepData_FieldList>  >(m,"StepData_FieldList",R"#(Describes a list of fields, in a general way This basic class is for a null size list Subclasses are for 1, N (fixed) or Dynamic sizes)#");
    ...
    py::class_<StepData_FieldListN ,std::unique_ptr<StepData_FieldListN>  , StepData_FieldList >(m,"StepData_FieldListN",R"#(Describes a list of fields, in a general way This basic class is for a null size list Subclasses are for 1, N (fixed) or Dynamic sizes)#");
    ...
    py::class_<StepData ,std::unique_ptr<StepData>  >(m,"StepData",R"#(Gives basic data definition for Step Interface. Any class of a data model described in EXPRESS Language is candidate to be managed by a Step Interface)#");
    py::class_<StepData_HeaderTool ,std::unique_ptr<StepData_HeaderTool>  >(m,"StepData_HeaderTool",R"#(HeaderTool exploits data from Header to build a Protocol : it uses the Header Entity FileSchema to do this. It builds a Protocol from the Global List of Protocols stored in the Library ReaderLib)#");
    ...
    py::class_<StepData_DescrProtocol ,opencascade::handle<StepData_DescrProtocol>  , StepData_FileProtocol >(m,"StepData_DescrProtocol",R"#(A DescrProtocol is a protocol dynamically (at execution time) defined with : - a list of resources (inherits FileProtocol) - a list of entity descriptions i.e. it can be defined with only C++ writing to initialize it Its initialization must : - set its schema name - define its resources (which can also be other DescrProtocol) - define its entity descriptions - record it in the system by calling RecordLibA DescrProtocol is a protocol dynamically (at execution time) defined with : - a list of resources (inherits FileProtocol) - a list of entity descriptions i.e. it can be defined with only C++ writing to initialize it Its initialization must : - set its schema name - define its resources (which can also be other DescrProtocol) - define its entity descriptions - record it in the system by calling RecordLibA DescrProtocol is a protocol dynamically (at execution time) defined with : - a list of resources (inherits FileProtocol) - a list of entity descriptions i.e. it can be defined with only C++ writing to initialize it Its initialization must : - set its schema name - define its resources (which can also be other DescrProtocol) - define its entity descriptions - record it in the system by calling RecordLib)#");
    py::class_<StepData_StepDumper ,std::unique_ptr<StepData_StepDumper>  >(m,"StepData_StepDumper",R"#(Provides a way to dump entities processed through STEP, with these features : - same form as for writing a STEP File (because it is clear and compact enough, even if the names of the fields do not appear) : thus, no additionnal resource is required - possibility to look for an entity itself (only its Type or with its content), an entity and it shared items (one level) or all the entities its refers to, directly or recursively.)#");
    py::class_<StepData_StepReaderTool ,std::unique_ptr<StepData_StepReaderTool>  , Interface_FileReaderTool >(m,"StepData_StepReaderTool",R"#(Specific FileReaderTool for Step; works with FileReaderData provides references evaluation, plus access to litteral data and specific methods defined by FileReaderTool Remarks : works with a ReaderLib to load Entities)#");
    ...
    py::class_<StepData_EnumTool ,std::unique_ptr<StepData_EnumTool>  >(m,"StepData_EnumTool",R"#(This class gives a way of conversion between the value of an enumeration and its representation in STEP An enumeration corresponds to an integer with reserved values, which begin to 0 In STEP, it is represented by a name in capital letter and limited by two dots, e.g. .UNKNOWN.)#");
    ...
    py::class_<StepData_FieldList1 ,std::unique_ptr<StepData_FieldList1>  , StepData_FieldList >(m,"StepData_FieldList1",R"#(Describes a list of ONE field)#");
    ...
    py::class_<StepData_DescrReadWrite ,opencascade::handle<StepData_DescrReadWrite>  , StepData_ReadWriteModule >(m,"StepData_DescrReadWrite",R"#()#");
    ...
    py::class_<StepData_DescrGeneral ,opencascade::handle<StepData_DescrGeneral>  , StepData_GeneralModule >(m,"StepData_DescrGeneral",R"#(Works with a Protocol by considering its entity descriptionsWorks with a Protocol by considering its entity descriptionsWorks with a Protocol by considering its entity descriptions)#");

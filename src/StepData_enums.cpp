...
// classes forward declarations only
    ...
    py::class_<StepData_EnumTool , shared_ptr<StepData_EnumTool>  >(m,"StepData_EnumTool",R"#(This class gives a way of conversion between the value of an enumeration and its representation in STEP An enumeration corresponds to an integer with reserved values, which begin to 0 In STEP, it is represented by a name in capital letter and limited by two dots, e.g. .UNKNOWN.)#");
    ...
    py::class_<StepData_HeaderTool , shared_ptr<StepData_HeaderTool>  >(m,"StepData_HeaderTool",R"#(HeaderTool exploits data from Header to build a Protocol : it uses the Header Entity FileSchema to do this. It builds a Protocol from the Global List of Protocols stored in the Library ReaderLib)#");

...
// classes forward declarations only
    py::class_<Poly_CoherentNode ,std::unique_ptr<Poly_CoherentNode>  , gp_XYZ >(m,"Poly_CoherentNode",R"#(Node of coherent triangulation. Contains: Coordinates of a 3D point defining the node location 2D point coordinates List of triangles that use this Node Integer index, normally the index of the node in the original triangulation)#");
    py::class_<Poly_MakeLoops3D ,std::unique_ptr<Poly_MakeLoops3D>  >(m,"Poly_MakeLoops3D",R"#(None)#");
    py::class_<Poly_CoherentLink ,std::unique_ptr<Poly_CoherentLink>  >(m,"Poly_CoherentLink",R"#(Link between two mesh nodes that is created by existing triangle(s). Keeps reference to the opposite node of each incident triangle. The referred node with index "0" is always on the left side of the link, the one with the index "1" is always on the right side. It is possible to find both incident triangles using the method Poly_CoherentTriangulation::FindTriangle(). Any Link can store an arbitrary pointer that is called Attribute.)#");
    ...
    py::class_<Poly_CoherentTriangle ,std::unique_ptr<Poly_CoherentTriangle>  >(m,"Poly_CoherentTriangle",R"#(Data class used in Poly_CoherentTriangultion. Implements a triangle with references to its neighbours.)#");
    ...
    py::class_<Poly_MakeLoops2D ,std::unique_ptr<Poly_MakeLoops2D>  >(m,"Poly_MakeLoops2D",R"#(None)#");

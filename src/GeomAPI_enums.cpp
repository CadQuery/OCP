...
// classes forward declarations only
    ...
    py::class_<GeomAPI_IntSS ,std::unique_ptr<GeomAPI_IntSS>  >(m,"GeomAPI_IntSS",R"#(This class implements methods for computing the intersection curves between two surfaces. The result is curves from Geom. The "domain" used for a surface is the natural parametric domain unless the surface is a RectangularTrimmedSurface from Geom.)#");
    py::class_<GeomAPI_ExtremaCurveCurve ,std::unique_ptr<GeomAPI_ExtremaCurveCurve>  >(m,"GeomAPI_ExtremaCurveCurve",R"#(Describes functions for computing all the extrema between two 3D curves. An ExtremaCurveCurve algorithm minimizes or maximizes the distance between a point on the first curve and a point on the second curve. Thus, it computes start and end points of perpendiculars common to the two curves (an intersection point is not an extremum unless the two curves are tangential at this point). Solutions consist of pairs of points, and an extremum is considered to be a segment joining the two points of a solution. An ExtremaCurveCurve object provides a framework for: - defining the construction of the extrema, - implementing the construction algorithm, and - consulting the results. Warning In some cases, the nearest points between two curves do not correspond to one of the computed extrema. Instead, they may be given by: - a limit point of one curve and one of the following: - its orthogonal projection on the other curve, - a limit point of the other curve; or - an intersection point between the two curves.)#");
    py::class_<GeomAPI_IntCS ,std::unique_ptr<GeomAPI_IntCS>  >(m,"GeomAPI_IntCS",R"#(This class implements methods for computing intersection points and segments between a)#");
    py::class_<GeomAPI_ProjectPointOnSurf ,std::unique_ptr<GeomAPI_ProjectPointOnSurf>  >(m,"GeomAPI_ProjectPointOnSurf",R"#(This class implements methods for computing all the orthogonal projections of a point onto a surface.)#");
    py::class_<GeomAPI_ProjectPointOnCurve ,std::unique_ptr<GeomAPI_ProjectPointOnCurve>  >(m,"GeomAPI_ProjectPointOnCurve",R"#(This class implements methods for computing all the orthogonal projections of a 3D point onto a 3D curve.)#");

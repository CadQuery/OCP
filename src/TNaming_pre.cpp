...
// classes forward declarations only
    ...
    py::class_<TNaming_Identifier , shared_ptr<TNaming_Identifier>  >(m,"TNaming_Identifier",R"#(None)#");
    py::class_<TNaming_Iterator , shared_ptr<TNaming_Iterator>  >(m,"TNaming_Iterator",R"#(A tool to visit the contents of a named shape attribute. Pairs of shapes in the attribute are iterated, one being the pre-modification or the old shape, and the other the post-modification or the new shape. This allows you to have a full access to all contents of an attribute. If, on the other hand, you are only interested in topological entities stored in the attribute, you can use the functions GetShape and CurrentShape in TNaming_Tool.)#");
    py::class_<TNaming_IteratorOnShapesSet , shared_ptr<TNaming_IteratorOnShapesSet>  >(m,"TNaming_IteratorOnShapesSet",R"#(None)#");
    py::class_<TNaming_Localizer , shared_ptr<TNaming_Localizer>  >(m,"TNaming_Localizer",R"#(None)#");
    py::class_<TNaming_Name , shared_ptr<TNaming_Name>  >(m,"TNaming_Name",R"#(store the arguments of Naming.)#");
    ...
    py::class_<TNaming_NamingTool , shared_ptr<TNaming_NamingTool>  >(m,"TNaming_NamingTool",R"#(None)#");
    py::class_<TNaming_NewShapeIterator , shared_ptr<TNaming_NewShapeIterator>  >(m,"TNaming_NewShapeIterator",R"#(Iterates on all the descendants of a shape)#");
    py::class_<TNaming_OldShapeIterator , shared_ptr<TNaming_OldShapeIterator>  >(m,"TNaming_OldShapeIterator",R"#(Iterates on all the ascendants of a shape)#");
    py::class_<TNaming_RefShape , shared_ptr<TNaming_RefShape>  >(m,"TNaming_RefShape",R"#(None)#");
    py::class_<TNaming_SameShapeIterator , shared_ptr<TNaming_SameShapeIterator>  >(m,"TNaming_SameShapeIterator",R"#(To iterate on all the label which contained a given shape.)#");
    py::class_<TNaming_Scope , shared_ptr<TNaming_Scope>  >(m,"TNaming_Scope",R"#(this class manage a scope of labels ===================================)#");
    py::class_<TNaming_Selector , shared_ptr<TNaming_Selector>  >(m,"TNaming_Selector",R"#(This class provides a single API for selection of shapes. This involves both identification and selection of shapes in the data framework. If the selected shape is modified, this selector will solve its identifications. This class is the user interface for topological naming resources. * The <IsIdentified> method returns (if exists) the NamedShape which contains a given shape. The definition of an identified shape is : a Shape handled by a NamedShape (this shape is the only one stored) , which has the TNaming_PRImITIVE evolution)#");
    py::class_<TNaming_ShapesSet , shared_ptr<TNaming_ShapesSet>  >(m,"TNaming_ShapesSet",R"#(None)#");
    py::class_<TNaming_Tool , shared_ptr<TNaming_Tool>  >(m,"TNaming_Tool",R"#(A tool to get information on the topology of a named shape attribute. This information is typically a TopoDS_Shape object. Using this tool, relations between named shapes are also accessible.)#");
    ...
    py::class_<TNaming_Translator , shared_ptr<TNaming_Translator>  >(m,"TNaming_Translator",R"#(only for Shape Copy test - to move in DNaming)#");

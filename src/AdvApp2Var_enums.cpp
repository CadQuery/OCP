...
// classes forward declarations only
    py::class_<AdvApp2Var_Patch ,std::unique_ptr<AdvApp2Var_Patch>  >(m,"AdvApp2Var_Patch",R"#(used to store results on a domain [Ui,Ui+1]x[Vj,Vj+1])#");
    py::class_<AdvApp2Var_SysBase ,std::unique_ptr<AdvApp2Var_SysBase>  >(m,"AdvApp2Var_SysBase",R"#(None)#");
    py::class_<AdvApp2Var_ApproxF2var ,std::unique_ptr<AdvApp2Var_ApproxF2var>  >(m,"AdvApp2Var_ApproxF2var",R"#(None)#");
    py::class_<AdvApp2Var_EvaluatorFunc2Var ,std::unique_ptr<AdvApp2Var_EvaluatorFunc2Var> ,Py_AdvApp2Var_EvaluatorFunc2Var >(m,"AdvApp2Var_EvaluatorFunc2Var",R"#(None)#");
    py::class_<AdvApp2Var_Network ,std::unique_ptr<AdvApp2Var_Network>  >(m,"AdvApp2Var_Network",R"#(None)#");
    py::class_<AdvApp2Var_MathBase ,std::unique_ptr<AdvApp2Var_MathBase>  >(m,"AdvApp2Var_MathBase",R"#(None)#");
    py::class_<AdvApp2Var_Criterion ,std::unique_ptr<AdvApp2Var_Criterion> ,Py_AdvApp2Var_Criterion >(m,"AdvApp2Var_Criterion",R"#(this class contains a given criterion to be satisfied)#");
    py::class_<AdvApp2Var_Framework ,std::unique_ptr<AdvApp2Var_Framework>  >(m,"AdvApp2Var_Framework",R"#(None)#");
    py::class_<AdvApp2Var_Node ,std::unique_ptr<AdvApp2Var_Node>  >(m,"AdvApp2Var_Node",R"#(used to store constraints on a (Ui,Vj) point)#");
    py::class_<AdvApp2Var_Iso ,std::unique_ptr<AdvApp2Var_Iso>  >(m,"AdvApp2Var_Iso",R"#(used to store constraints on a line U = Ui or V = Vj)#");
    py::class_<AdvApp2Var_Data ,std::unique_ptr<AdvApp2Var_Data>  >(m,"AdvApp2Var_Data",R"#(/)#");
    py::class_<AdvApp2Var_ApproxAFunc2Var ,std::unique_ptr<AdvApp2Var_ApproxAFunc2Var>  >(m,"AdvApp2Var_ApproxAFunc2Var",R"#(Perform the approximation of <Func> F(U,V) Arguments are : Num1DSS, Num2DSS, Num3DSS :The numbers of 1,2,3 dimensional subspaces OneDTol, TwoDTol, ThreeDTol: The tolerance of approximation in each subspaces OneDTolFr, TwoDTolFr, ThreeDTolFr: The tolerance of approximation on the boundarys in each subspaces [FirstInU, LastInU]: The Bounds in U of the Approximation [FirstInV, LastInV]: The Bounds in V of the Approximation FavorIso : Give preference to extract u-iso or v-iso on F(U,V) This can be usefull to optimize the <Func> methode ContInU, ContInV : Continuity waiting in u and v PrecisCode : Precision on approximation's error mesurement 1 : Fast computation and average precision 2 : Average computation and good precision 3 : Slow computation and very good precision MaxDegInU : Maximum u-degree waiting in U MaxDegInV : Maximum u-degree waiting in V Warning: MaxDegInU (resp. MaxDegInV) must be >= 2*iu (resp. iv) + 1, where iu (resp. iv) = 0 if ContInU (resp. ContInV) = GeomAbs_C0, = 1 if = GeomAbs_C1, = 2 if = GeomAbs_C2. MaxPatch : Maximun number of Patch waiting number of Patch is number of u span * number of v span Func : The external method to evaluate F(U,V) Crit : To (re)defined condition of convergence UChoice, VChoice : To define the way in U (or V) Knot insertion Warning: for the moment, the result is a 3D Surface so Num1DSS and Num2DSS must be equals to 0 and Num3DSS must be equal to 1. Warning: the Function of type EvaluatorFunc2Var from Approx must be a subclass of AdvApp2Var_EvaluatorFunc2Var)#");
    py::class_<AdvApp2Var_Context ,std::unique_ptr<AdvApp2Var_Context>  >(m,"AdvApp2Var_Context",R"#(contains all the parameters for approximation ( tolerancy, computing option, ...))#");

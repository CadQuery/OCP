...
// classes forward declarations only
    py::class_<IGESData_SpecificLib ,std::unique_ptr<IGESData_SpecificLib>  >(m,"IGESData_SpecificLib",R"#(None)#");
    ...
    py::class_<IGESData_WriterLib ,std::unique_ptr<IGESData_WriterLib>  >(m,"IGESData_WriterLib",R"#(None)#");
    ...
    py::class_<IGESData_ParamReader ,std::unique_ptr<IGESData_ParamReader>  >(m,"IGESData_ParamReader",R"#(access to a list of parameters, with management of read stage (owned parameters, properties, associativities) and current parameter number, read errors (which feed a Check), plus convenient facilities to read parameters, in particular : - first parameter is ignored (it repeats entity type), hence number 1 gives 2nd parameter, etc... - lists are not explicit, list-reading methods are provided which manage a current param. number - interpretation is made as possible (texts, reals, entities ...) (in particular, Reading a Real accepts an Integer))#");
    py::class_<IGESData_GlobalSection ,std::unique_ptr<IGESData_GlobalSection>  >(m,"IGESData_GlobalSection",R"#(Description of a global section (corresponds to file header) used as well in IGESModel, IGESReader and IGESWriter Warning : From IGES-5.1, a parameter is added : LastChangeDate (concerns transferred set of data, not the file itself) Of course, it can be absent if read from earlier versions (a default is then to be set to current date) From 5.3, one more : ApplicationProtocol (optional))#");
    ...
    py::class_<IGESData_IGESWriter ,std::unique_ptr<IGESData_IGESWriter>  >(m,"IGESData_IGESWriter",R"#(manages atomic file writing, under control of IGESModel : prepare text to be sent then sends it takes into account distinction between successive Sections)#");
    ...
    py::class_<IGESData_IGESReaderTool ,std::unique_ptr<IGESData_IGESReaderTool>  , Interface_FileReaderTool >(m,"IGESData_IGESReaderTool",R"#(specific FileReaderTool for IGES Parameters are accessed through specific objects, ParamReaders)#");
    py::class_<IGESData_HArray1OfIGESEntity ,std::unique_ptr<IGESData_HArray1OfIGESEntity>  >(m,"IGESData_HArray1OfIGESEntity",R"#()#");
    ...
    py::class_<IGESData_DirPart ,std::unique_ptr<IGESData_DirPart>  >(m,"IGESData_DirPart",R"#(litteral/numeric description of an entity's directory section, taken from file)#");
    ...
    py::class_<IGESData_IGESType ,std::unique_ptr<IGESData_IGESType>  >(m,"IGESData_IGESType",R"#(taken from directory part of an entity (from file or model), gives "type" and "form" data, used to recognize entity's type)#");
    py::class_<IGESData ,std::unique_ptr<IGESData>  >(m,"IGESData",R"#(basic description of an IGES Interface)#");
    ...
    py::class_<IGESData_IGESDumper ,std::unique_ptr<IGESData_IGESDumper>  >(m,"IGESData_IGESDumper",R"#(Provides a way to obtain a clear Dump of an IGESEntity (distinct from normalized output). It works with tools attached to Entities, as for normalized Reade and Write)#");

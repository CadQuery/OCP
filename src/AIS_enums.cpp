...
// enums
    ...
    py::enum_<AIS_ManipulatorMode>(m, "AIS_ManipulatorMode",R"#(Mode to make definite kind of transformations with AIS_Manipulator object.)#")
        ...
        .value("AIS_MM_Scaling",AIS_ManipulatorMode::AIS_MM_Scaling).export_values();
...
// classes forward declarations only
    ...
    py::class_<AIS_GraphicTool ,std::unique_ptr<AIS_GraphicTool>  >(m,"AIS_GraphicTool",R"#(None)#");
    ...
    py::class_<AIS_ManipulatorObjectSequence ,std::unique_ptr<AIS_ManipulatorObjectSequence>  >(m,"AIS_ManipulatorObjectSequence",R"#()#");
    py::class_<AIS_AnimationTimer ,opencascade::handle<AIS_AnimationTimer>  , Standard_Transient >(m,"AIS_AnimationTimer",R"#(Auxiliary class defining the animation timer.Auxiliary class defining the animation timer.)#");
    ...
    py::class_<AIS_LocalContext ,opencascade::handle<AIS_LocalContext>  , Standard_Transient >(m,"AIS_LocalContext",R"#(Defines a specific context for selection. It becomes possible to: + Load InteractiveObjects with a mode to be activated + associate InteractiveObjects with a set of temporary selectable Objects.... + + activate StandardMode of selection for Entities inheriting BasicShape from AIS (Selection Of vertices, edges, wires,faces... + Add Filters acting on detected owners of sensitive primitivesDefines a specific context for selection. It becomes possible to: + Load InteractiveObjects with a mode to be activated + associate InteractiveObjects with a set of temporary selectable Objects.... + + activate StandardMode of selection for Entities inheriting BasicShape from AIS (Selection Of vertices, edges, wires,faces... + Add Filters acting on detected owners of sensitive primitivesDefines a specific context for selection. It becomes possible to: + Load InteractiveObjects with a mode to be activated + associate InteractiveObjects with a set of temporary selectable Objects.... + + activate StandardMode of selection for Entities inheriting BasicShape from AIS (Selection Of vertices, edges, wires,faces... + Add Filters acting on detected owners of sensitive primitives)#");
    ...
    py::class_<AIS_Animation ,opencascade::handle<AIS_Animation>  , Standard_Transient >(m,"AIS_Animation",R"#(Class represents single animation. It is defined with: - Start time on the timeline started from 0, in seconds - Duration, in seconds - virtual method Update() for handling an updateClass represents single animation. It is defined with: - Start time on the timeline started from 0, in seconds - Duration, in seconds - virtual method Update() for handling an update)#");
    ...
    py::class_<AIS_LocalStatus ,opencascade::handle<AIS_LocalStatus>  , Standard_Transient >(m,"AIS_LocalStatus",R"#(Stored Info about temporary objects.Stored Info about temporary objects.)#");
    ...
    py::class_<AIS_Dimension ,opencascade::handle<AIS_Dimension>  , AIS_InteractiveObject >(m,"AIS_Dimension",R"#(AIS_Dimension is a base class for 2D presentations of linear (length, diameter, radius) and angular dimensions.AIS_Dimension is a base class for 2D presentations of linear (length, diameter, radius) and angular dimensions.AIS_Dimension is a base class for 2D presentations of linear (length, diameter, radius) and angular dimensions.)#");
    ...
    py::class_<AIS_ParallelRelation ,opencascade::handle<AIS_ParallelRelation>  , AIS_Relation >(m,"AIS_ParallelRelation",R"#(A framework to display constraints of parallelism between two or more Interactive Objects. These entities can be faces or edges.A framework to display constraints of parallelism between two or more Interactive Objects. These entities can be faces or edges.A framework to display constraints of parallelism between two or more Interactive Objects. These entities can be faces or edges.)#");
    ...
    py::class_<AIS_SymmetricRelation ,opencascade::handle<AIS_SymmetricRelation>  , AIS_Relation >(m,"AIS_SymmetricRelation",R"#(A framework to display constraints of symmetricity between two or more datum Interactive Objects. A plane serves as the axis of symmetry between the shapes of which the datums are parts.A framework to display constraints of symmetricity between two or more datum Interactive Objects. A plane serves as the axis of symmetry between the shapes of which the datums are parts.A framework to display constraints of symmetricity between two or more datum Interactive Objects. A plane serves as the axis of symmetry between the shapes of which the datums are parts.)#");
    py::class_<AIS_MidPointRelation ,opencascade::handle<AIS_MidPointRelation>  , AIS_Relation >(m,"AIS_MidPointRelation",R"#(presentation of equal distance to point myMidPointpresentation of equal distance to point myMidPointpresentation of equal distance to point myMidPoint)#");
    ...
    py::class_<AIS_IdenticRelation ,opencascade::handle<AIS_IdenticRelation>  , AIS_Relation >(m,"AIS_IdenticRelation",R"#(Constructs a constraint by a relation of identity between two or more datums figuring in shape Interactive Objects.Constructs a constraint by a relation of identity between two or more datums figuring in shape Interactive Objects.Constructs a constraint by a relation of identity between two or more datums figuring in shape Interactive Objects.)#");
    ...
    py::class_<AIS_FixRelation ,opencascade::handle<AIS_FixRelation>  , AIS_Relation >(m,"AIS_FixRelation",R"#(Constructs and manages a constraint by a fixed relation between two or more interactive datums. This constraint is represented by a wire from a shape - point, vertex, or edge - in the first datum and a corresponding shape in the second. Warning: This relation is not bound with any kind of parametric constraint : it represents the "status" of an parametric object.Constructs and manages a constraint by a fixed relation between two or more interactive datums. This constraint is represented by a wire from a shape - point, vertex, or edge - in the first datum and a corresponding shape in the second. Warning: This relation is not bound with any kind of parametric constraint : it represents the "status" of an parametric object.Constructs and manages a constraint by a fixed relation between two or more interactive datums. This constraint is represented by a wire from a shape - point, vertex, or edge - in the first datum and a corresponding shape in the second. Warning: This relation is not bound with any kind of parametric constraint : it represents the "status" of an parametric object.)#");
    ...
    py::class_<AIS_MinRadiusDimension ,opencascade::handle<AIS_MinRadiusDimension>  , AIS_EllipseRadiusDimension >(m,"AIS_MinRadiusDimension",R"#(-- Ellipse Min radius dimension of a Shape which can be Edge or Face (planar or cylindrical(surface of extrusion or surface of offset))-- Ellipse Min radius dimension of a Shape which can be Edge or Face (planar or cylindrical(surface of extrusion or surface of offset))-- Ellipse Min radius dimension of a Shape which can be Edge or Face (planar or cylindrical(surface of extrusion or surface of offset)))#");
    ...
    py::class_<AIS_PerpendicularRelation ,opencascade::handle<AIS_PerpendicularRelation>  , AIS_Relation >(m,"AIS_PerpendicularRelation",R"#(A framework to display constraints of perpendicularity between two or more interactive datums. These datums can be edges or faces.A framework to display constraints of perpendicularity between two or more interactive datums. These datums can be edges or faces.A framework to display constraints of perpendicularity between two or more interactive datums. These datums can be edges or faces.)#");
...
// ./opencascade/AIS_DataMapOfSelStat.hxx
    preregister_template_NCollection_DataMap<opencascade::handle<SelectMgr_SelectableObject>, opencascade::handle<AIS_LocalStatus>, TColStd_MapTransientHasher>(m,"AIS_DataMapOfSelStat");  
